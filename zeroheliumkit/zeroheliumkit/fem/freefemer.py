...
class FreeFEM():
    ...
    def run(self,
            print_log=False,
            freefem_path='\\'):
            # freefem_path='C:\\Program Files (x86)\\FreeFem++\\'):
        try:
            edp_name = self.config["meshfile"]
            ff_executable = shutil.which('freefem++')  
            print("Résolu freefem++ :", ff_executable)    
            bashCommand_ff = [ff_executable, edp_name + '.edp']
            env = os.environ.copy()
            env['PATH'] += freefem_path
            self.dirname = os.path.abspath(self.dirname)
            print("bashCommand_ff:", bashCommand_ff)
            print("cwd:", self.dirname)
            print("env['PATH']:", env['PATH'])
            process = subprocess.Popen(bashCommand_ff, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, cwd=self.dirname)
            logs = ''
