...
class FullModel(EOMSolver, PositionSolver, PotentialVisualization):
    ...
    def get_electron_positions(self, n_electrons: int, electron_initial_positions: Optional[ArrayLike] = None, verbose: bool = False,
                               suppress_warnings: bool = False,   gradient_tolerance: float = 1e-1, maxiter: int = 200, ftol: float = 1e-30) -> dict:
        ...
        Args:
            ...
            gradient_tolerance (int, optional): This is the maximum gradient norm that is axceptable for convergence. # Units are eV/m. Defaults to 1e-1.
            maxiter (int, optional): Maximum number of iterations for the minimization. Defaults to 200.
        ...
        trap_minimizer_options = {'method': 'L-BFGS-B',
                                  'jac': self.grad_total,
                                  'options': {'disp': False, 'gtol': gradient_tolerance, 'maxls': maxiter, 'ftol': ftol},
                                #   'callback': self.CM.monitor_convergence}
                                   'callback': callback_with_f_tracking}
        ...
        if "factr" in res['message'].lower():
            print(res['message'])
            if len(self.F_history) >= 2:
                F_rel_diff = abs(self.F_history[-1] - self.F_history[-2]) / max(abs(self.F_history[-2]), 1)
                print(f"[Convergence diagnostic] Final relative variation of F: {F_rel_diff:.3e}")
            else:
                print("[Convergence diagnostic] Not enough data to compute relative variation of F.")
    ...
    def animate_voltage_sweep(self, fig, ax, list_of_voltages: list, list_of_electron_positions: list, coor: tuple = (0, 0), dxdy: tuple = (2, 2), frame_interval_ms: int = 10) -> matplotlib.animation.FuncAnimation:
        ...
        for k, electrode in enumerate(initial_voltages.keys()):
            offset_x = 0.15 * (xmax - xmin)
            offset_y = 0.1 * (ymax - ymin)
            text_boxes.append(ax.text(xmin - offset_x,
                                       ymax - k * offset_y,
                                       f"{electrode} = {initial_voltages[electrode] * 1e3:.1f} mV",  # conversion en mV
                                       ha='right', va='top'))
        ...
        def update(frame):
            ...
            for k, electrode in enumerate(voltages.keys()):
                text_boxes[k].set_text(
                    f"{electrode} = {voltages[electrode] * 1e3:.1f} mV")
